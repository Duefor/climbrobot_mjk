...
{
    Matrix4d T[7];
    Matrix4d T_total = Matrix4d::Identity();
    ...
    // 提取位置
    Vector3d position = T_total.block<3,1>(0,3);
    cout << "位置: X=" << position(0) << " Y=" << position(1) << " Z=" << position(2) << endl;
    
    // 提取姿态（旋转矩阵）
    Matrix3d R = T_total.block<3,3>(0,0);
    ...
    cout << "欧拉角(RPY-ZYX): rx=" << rx << " ry=" << ry << " rz=" << rz << endl;
    cout << "角度制: rx=" << rx*180/M_PI << "° ry=" << ry*180/M_PI << "° rz=" << rz*180/M_PI << "°" << endl;
}
...
int main(int argc, char** argv)
{
    ...
    while (ros::ok()) {
        ...
        if (pose_received) {
            ...
            {
                std::lock_guard<std::mutex> lock(state_mtx);
                x = latest_pose.position[0];
                y = latest_pose.position[1];
                z = latest_pose.position[2];
                RX = latest_pose.position[3];
                RY = latest_pose.position[4];
                RZ = latest_pose.position[5];
            }
