...
class ScanEnvironmentServer:
    ...
    def __init__(self):
        ...
        self.window_size = rospy.get_param("~window_size", 7) # 参数含义：PCA 邻域半径，越大越平滑但越模糊，建议奇数以中心对称
        ...
        self.robust_outlier_deg = rospy.get_param("~robust_outlier_deg", 50.0) # 夹角超过该值的法向量被视为离群点，替换为邻域参考并降权质量
        ...
        self.debug_image_enabled = rospy.get_param("~debug_image_enabled", True)
        self.debug_image_prefix = rospy.get_param("~debug_image_prefix", "scan_environment")
        self.debug_arrow_min_px = rospy.get_param("~debug_arrow_min_px", 18)
        self.debug_arrow_stride = rospy.get_param("~debug_arrow_stride", 1)
        default_debug_dir = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "debug_images")
        )
        self.debug_image_dir = rospy.get_param("~debug_image_dir", default_debug_dir)
    ...
    # ------------------------------------------------------------------
    #  保存深度伪彩图 + 法向箭头，方便脱离 RViz 查看扫描结果
    # ------------------------------------------------------------------
    def _depth_to_debug_bgr(self, depth):
        """把深度图渲染成 OpenCV BGR 伪彩图，无效深度显示为黑色。"""
        valid = depth > 0.0
        if not np.any(valid):
        ...
        depth_u8[valid] = np.clip(depth_norm[valid] * 255.0, 0, 255).astype(np.uint8)
        debug = cv2.applyColorMap(depth_u8, cv2.COLORMAP_TURBO)
        debug[~valid] = (0, 0, 0)
        return debug

    def _project_camera_point(self, point_cam, image_shape):
        """相机坐标系 3D 点投影到像素坐标。"""
        if point_cam[2] <= 1e-6:
            return None
    ...
    def _draw_debug_arrow(self, image, origin_cam, normal_cam, quality):
        """在深度伪彩图上画出一个清晰可见的法向箭头。"""
        start = self._project_camera_point(origin_cam, image.shape)
        if start is None:
            return

        tip_cam = origin_cam + normal_cam * self.arrow_length
        end = self._project_camera_point(tip_cam, image.shape)
        if end is None:
            end = start + np.array([normal_cam[0], normal_cam[1]], dtype=np.float64)

        delta = end - start
        length = float(np.linalg.norm(delta))
        if length < 1e-6:
            delta = np.array([normal_cam[0], normal_cam[1]], dtype=np.float64)
            length = float(np.linalg.norm(delta))
        if length < 1e-6:
            delta = np.array([0.0, -1.0], dtype=np.float64)
            length = 1.0

        min_len = max(1.0, float(self.debug_arrow_min_px))
        if length < min_len:
            end = start + delta / length * min_len

        H, W = image.shape[:2]
        end[0] = np.clip(end[0], 0, W - 1)
        end[1] = np.clip(end[1], 0, H - 1)

        alpha = 0.3 + 0.6 * float(quality)
        color = (
            int(255 * alpha),
            int(80 * (1.0 - alpha)),
            int(255 * (1.0 - alpha)),
        ...
        cv2.circle(image, tuple(np.round(start).astype(np.int32)), 2, (255, 255, 255), -1)

    def _save_debug_image(self, depth, debug_arrows):
        """保存一张包含法向箭头的调试图。"""
        if not self.debug_image_enabled:
            return

        debug = self._depth_to_debug_bgr(depth)
        stride = max(1, int(self.debug_arrow_stride))
        for origin_cam, normal_cam, quality in debug_arrows[::stride]:
            self._draw_debug_arrow(debug, origin_cam, normal_cam, quality)

        stamp = int(rospy.Time.now().to_nsec() / 1000000)
        path = os.path.join(self.debug_image_dir, "{}_{}.jpg".format(self.debug_image_prefix, stamp))
        if cv2.imwrite(path, debug):
            rospy.loginfo("scan_environment_server: debug image saved: %s", path)
        else:
            rospy.logwarn("scan_environment_server: debug image save failed: %s", path)
    ...
    def execute_scan(self, goal):
        ...
        for r in range(grid_r):
            for c in range(grid_c):
                ...
                debug_arrows.append((origin_cam.copy(), normal_cam.copy(), q))
                marker_id += 1
        ...
        self._save_debug_image(depth, debug_arrows)
