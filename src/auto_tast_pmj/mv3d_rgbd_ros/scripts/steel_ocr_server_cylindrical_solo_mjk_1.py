...
class SteelStampOcrSoloServer:
    ...
    def __init__(self):
        ...
        self.center_depth_band_mm = float(rospy.get_param('~center_depth_band_mm', 15.0))
        self.center_depth_min_points = int(rospy.get_param('~center_depth_min_points', 10))
    ...
    def handle_req(self, _req):
        ...
        for line in detections:
            ...
            center_band_mask = valid_mask & (
                np.abs(roi_depth.astype(np.float64) - median_depth) <= self.center_depth_band_mm
            )
            if np.count_nonzero(center_band_mask) >= self.center_depth_min_points:
                point_mask = center_band_mask
            else:
            ...
            u_valid = u_grid[point_mask].astype(np.float64)          # 有效像素的 u 坐标
            v_valid = v_grid[point_mask].astype(np.float64)          # 有效像素的 v 坐标
            z_valid = roi_depth[point_mask].astype(np.float64) / 1000.0  # 毫米→米
            ...
            # --- 4. 深度带内点云中值 = 鲁棒 3D 锚点 ---
            # 先按深度中位数剔除跳变层，再取坐标中位数，减少个别字符深度跳变造成的中心点偏移。
            best_point = np.median(filtered_points, axis=0)  # (3,) — 中位数 x,y,z
