...
class SteelStampOcrSoloServer:
    ...
    def __init__(self):
        ...
        # 基座负方向，用于约束法向
        self.base_negative_axis = np.asarray(rospy.get_param('~base_negative_axis', [1.0, 0.0, 0.0]), dtype=np.float64)
        ...
        self.base_negative_axis = self.safe_normalize(self.base_negative_axis, np.array([-1.0, 0.0, 0.0], dtype=np.float64))

        self.latest_data = {'rgb': None, 'depth': None, 'info': None}
        self.data_lock = threading.Lock()

        # 加载 OCR 模型
        self.det_model_path = '/workspace/PaddleOCR/inference/det_steel_1280x720'
        self.rec_model_path = '/workspace/PaddleOCR/inference/rec_single_char'
        self.ocr = PaddleOCR(
            use_angle_cls=True,
            lang='en',
            use_gpu=False,
    ...
    def handle_req(self, _req):
        ...
        for line in detections:
            ...
            # ==========================================================
            # 步骤 6：法向量 + 3D锚点 → 6-DOF 姿态 (geometry_msgs/Pose)
            #     - 强制法向朝向 base_negative_axis（保证工具头指向工件表面）
            #     - Gram-Schmidt 正交化: Z=法向, 构造正交的 X,Y 切向
            ...
            valid_entries.append({
                'text':   text,        # 识别的字符文本
                'pose':   pose,        # geometry_msgs/Pose (position + quaternion)
                'point':  best_point,  # 3D 锚点, (3,) numpy array (base_link 系)
                'normal': best_normal, # 曲面法向量, (3,) numpy array (已归一化)
            })
    ...
    def generate_bspline_trajectory(self, valid_entries):
        ...
        for pt, nrm in zip(smooth_points, smooth_normals):
            dense_poses.append(self.normal_to_pose(pt, nrm))
