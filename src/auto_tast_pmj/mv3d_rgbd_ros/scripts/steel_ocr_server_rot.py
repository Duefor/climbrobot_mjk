...
class SteelStamp3DServer:
    def __init__(self):
        ...
        # 1.2 PaddleOCR 初始化
        # 检测模型和识别模型路径
        self.det_model_path = "/home/barry/workspace/ws_moveit/PaddleOCR/inference/det_steel_v1/"
    ...
    def handle_req(self, req):
        ...
        try:
            ...
            # =================================================================
            # 【核心修改 1/2】：强制旋转图像 180 度
            # 注意：RGB 和 Depth 必须做完全相同的旋转，否则 3D 对齐会错乱！
            # =================================================================
            rgb_arr = cv2.rotate(rgb_arr, cv2.ROTATE_180)
            depth_arr = cv2.rotate(depth_arr, cv2.ROTATE_180)
            rospy.loginfo("Image rotated 180 degrees for detection.")

        except Exception as e:
            response.message = f"Image parse error: {e}"
            return response

        # 3. 执行 PaddleOCR (此时送进去的是转正后的图)
        ...
        try:
            ...
            # =================================================================
            # 【核心修改 2/2】：修正 3D 坐标系
            # 原理：图像旋转180度 等价于 相机绕光轴(Z轴)旋转了180度。
            # 我们生成的点云是在这个"旋转后的虚拟相机系"里的，所以需要把这个旋转量
            # 乘到 T_base_cam 里，才能算回正确的 base_link 坐标。
            # =================================================================
            # 构造绕 Z 轴 180 度的旋转矩阵 (4x4)
            correction_matrix = np.eye(4)
            correction_matrix[:3, :3] = R.from_euler('z', 180, degrees=True).as_matrix()
            
            # 右乘修正矩阵 (T_base_new = T_base_old * T_rot_180)
            T_base_cam = T_base_cam @ correction_matrix
        ...
        for line in detected_list:
            ...
            # 调用 3D 计算 (传入的是修正后的 T_base_cam 和 旋转后的 depth)
            ...
            if pose_msg is not None:
                ...
                arrow_marker.id = success_count * 2
                ...
                text_marker.id = success_count * 2 + 1
